...
import MsCustDetail from '@/components/ms/cust/MsCustDetail';
import MsCustSearch from '@/components/ms/cust/MsCustSearch';

// API Call Function
import { apiGetMasterList, apiGetSelectDccodeList } from '@/api/ms/apiMsCust';
...
const MsCust = () => {
	...
	// 주출고처
	const [dlvDcList, setDlvDcList] = useState([]);
	...
	const apiGetMasterListRun = () => {
		...
		const params = {
			...
			dlvDccode: form.getFieldValue('dlvDccode'),
	...
	// 주출고처 목록세팅
	useEffect(() => {
		apiGetSelectDccodeList().then(res => {
			setDlvDcList(res.data);
		});
	}, []);
