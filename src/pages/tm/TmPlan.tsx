	...
	const executeDispatch = useCallback(
		async (options?: { isRerun?: boolean }) => {
			...
			if (pendingCriteria) {
				...
				// 배차옵션 변경 시 재배차(AUTO)로 실행, 그 외에는 배차조정(ADJUST) 결과 조회
				const res = options?.isRerun
					? await setDispatch({
							dccode: pendingAdjustParams.dccode || '',
							deliveryDate: pendingAdjustParams.deliveryDate || storedDeliveryDate || '',
							deliveryType: (pendingAdjustParams.deliveryType as TmSetDispatchReqDto['deliveryType']) || 'WD',
						})
					: await getAdjustPlan({
							deliveryDate: pendingAdjustParams.deliveryDate || storedDeliveryDate || '',
							dccode: pendingAdjustParams.dccode,
				...
				if (res?.statusCode === 0) {
					...
					concurrencyDispatchInfoRef.current = {
						isPartialSave: options?.isRerun ? false : true,
						partialSaveVehicles: [],
						partialSaveUnassignedOrdersOrigin: [],
						partialSaveDeletedVehicles: [],
					};
					...
					if (!hasNeedRecalculation) {
						const msg = options?.isRerun ? '변경된 옵션으로 재배차가 완료되었습니다.' : '배차조정 결과를 불러왔습니다.';
						showAlert('알림', msg);
					}
					return true;
				}
