/*
 ############################################################################
 # FiledataField	: TmTrxCalculationReport.tsx
 # Description		: 운송비정산서
...
import TmTrxCalculationReportDetail from '@/components/tm/trxCalculationReport/TmTrxCalculationReportDetail';
import TmTrxCalculationReportSearch from '@/components/tm/trxCalculationReport/TmTrxCalculationReportSearch';
...
// API
import { apiPostMasterList, apiPostWorkDay } from '@/api/tm/apiTmTrxCalculationReport';

// Hooks
import { useThrottle } from '@/hooks/useThrottle';

const TmCalculationReport = () => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const searchWorkDay = async () => {
		...
		apiPostWorkDay(params).then(res => {
			if (res.data.length > 0) {
				const workDay = t('lbl.ONBIZDAYS') + ' ' + String(res.data[0].workDay) + '일 ';
				setWorkDay(workDay);
			}
		});
	};
	...
	return (
		<>
			...
			<SearchFormResponsive form={searchForm} initialValues={searchBox}>
				<TmTrxCalculationReportSearch form={searchForm} />
			</SearchFormResponsive>

			{/* 상세 영역 정의 */}
			<TmTrxCalculationReportDetail
				ref={refs}
				gridData={gridData}
...
export default TmCalculationReport;
