/*
 ############################################################################
 # FiledataField	: TmAmountByCar.tsx
...
// API Call Function
import { apiGetMasterList } from '@/api/tm/apiTmAmountByCar';
import TmAmountByCarDetail from '@/components/tm/amountbycar/TmAmountByCarDetail';
import TmAmountByCarSearch from '@/components/tm/amountbycar/TmAmountByCarSearch';
...
const TmAmountByCar = () => {
	...
	const searchMasterList = async () => {
		...
		apiGetMasterList(params).then(res => {
			const rows = res.data || [];
			// 각 행에 대해 sttlItemType 계산해서 넣기
			rows.forEach((row: any, index: any) => {
				if (row.sttlItemCd) {
					const code = getCommonCodebyCd('TM_CALC_ITEM', row.sttlItemCd);
					const data1 = code?.data1 || '';
					row.sttlItemType = data1;
				}
			});
	...
	return (
		<>
			...
			<SearchFormResponsive form={form} initialValues={searchBox} onValuesChange={onValuesChange}>
				<TmAmountByCarSearch form={form} />
			</SearchFormResponsive>

			{/* 화면 상세 영역 정의 */}
			<TmAmountByCarDetail ref={refs} data={gridData} totalCnt={totalCnt} callBackFn={searchMasterList} form={form} />
		</>
	);
};

export default TmAmountByCar;
