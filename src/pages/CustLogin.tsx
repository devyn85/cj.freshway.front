...
const CustLogin = () => {
	...
	const doLogin = async (verificationYn?: string) => {
		...
		if (saveId) {
			localStorage.setItem('savedUserId', loginForm.getFieldValue('username') + '_WAYLO');
		} else {
			localStorage.removeItem('savedUserId');
		}

		// redirect 하기 위한 로그인 페이지 저장
		localStorage.setItem('savedLoginPage', '/custLogin');

		const param = {
			username: loginForm.getFieldValue('username') + '_WAYLO',
				...
				switch (res.statusCode) {
					...
					case 40: // 비밀번호 사용기간 만료 전 안내
						showConfirm(
							...
							() => {
								// 비밀번호 변경 페이지로 이동
								return navigate('/PwdChange', { state: { userId: loginForm.getFieldValue('username') + '_WAYLO' } });
							},
							() => {
								doLogin('N');
							},
