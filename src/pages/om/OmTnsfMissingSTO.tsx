...
const OmTnsfMissingSTO = () => {
	...
	const savePrintList = async () => {
		...
		showConfirm(null, t('msg.MSG_COM_PRT_003'), async () => {
			// 1. 리포트 파일명
			const fileName = 'OM_TnsfMissingSTO.mrd';
			...
			const reportData = checkedItems.map((item: any, index: number) => ({
				orgDocdt: item.orgDocdt ? dayjs(item.orgDocdt, 'YYYYMMDD').format('MM월 DD일') : '',
				herderTxt: `${item.dcname} -> ${item.orderDcname}`,
				orderDccode: item.orderDccode,
				orderDcname: item.orderDcname,
				dcname: item.dcname,
				loc: item.loc, // 순번으로 표시
