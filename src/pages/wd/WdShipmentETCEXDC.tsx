/*
 ############################################################################
 # FiledataField	: WdShipmentETCEXDC.tsx
 # Description		: 출고 > 기타출고 > 외부센터매각출고처리
 # Author					: JiHoPark
 # Since					: 2025.09.11.
 ############################################################################
*/
...
import WdShipmentETCEXDCSearch from '@/components/wd/shipmentETCEXDC/WdShipmentETCEXDCSearch';

import WdShipmentETCEXDCDetail1 from '@/components/wd/shipmentETCEXDC/WdShipmentETCEXDCDetail1';
import WdShipmentETCEXDCDetail2 from '@/components/wd/shipmentETCEXDC/WdShipmentETCEXDCDetail2';
import WdShipmentETCEXDCDetail3 from '@/components/wd/shipmentETCEXDC/WdShipmentETCEXDCDetail3';
import WdShipmentETCEXDCDetail4 from '@/components/wd/shipmentETCEXDC/WdShipmentETCEXDCDetail4';
...
// API
import { apiGetMasterList, apiGetMasterList3, apiGetMasterList4 } from '@/api/wd/apiWdShipmentETCEXDC';
...
const WdShipmentETCEXDC = () => {
	...
	// 외부비축 매각출고처리에서 사용되는 발생사유 코드 filter
	const dsReasonCode = getCommonCodeList('PROCESSREASON_ETC', t('lbl.SELECT'), '').filter(item => {
		const comCd = item.comCd;
		const data4 = item.data4;
		// 발생사유
		return comCd === '' || data4 === '2170';
	});
	...
	const tabItemList = [
		{
			...
			children: (
				<Splitter
					...
					items={[
						<WdShipmentETCEXDCDetail1
							key="WdShipmentETCEXDCDetail1"
							ref={gridRefs1}
							form={gridForm}
							data={gridData1}
							totalCnt={totalCnt1}
						...
						/>,
						<WdShipmentETCEXDCDetail2
							key="WdShipmentETCEXDCDetail2"
							ref={gridRefs2}
							form={gridForm}
							data={gridData2}
							totalCnt={totalCnt2}
		...
		{
			key: '2',
			label: t('lbl.APPROVAL_WD'),
			children: (
				<WdShipmentETCEXDCDetail3
					ref={gridRefs3}
					form={gridForm}
					data={gridData3}
					totalCnt={totalCnt3}
		...
		{
			key: '3',
			label: t('lbl.PROCESS_WD'),
			children: (
				<WdShipmentETCEXDCDetail4
					ref={gridRefs4}
					form={gridForm}
					data={gridData4}
					totalCnt={totalCnt4}
	...
	return (
		<>
			...
			<SearchFormResponsive form={searchForm} initialValues={searchBox}>
				<WdShipmentETCEXDCSearch form={searchForm} currentTabKey={currentTabKey} />
...
export default WdShipmentETCEXDC;
