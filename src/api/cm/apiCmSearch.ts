...
const settingSelectData = (
	...
	let searchName = commUtil.convertCodeWithName(val[0].code, val[0].name, returnValueFormat);
	let searchCode = val[0].code;

	for (let i = 1; i < val.length; i++) {
		searchName += `,${commUtil.convertCodeWithName(val[i].code, val[i].name, returnValueFormat)}`;
