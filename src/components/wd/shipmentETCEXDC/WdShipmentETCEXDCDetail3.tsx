/*
 ############################################################################
 # FiledataField	: WdShipmentETCEXDCDetail3.tsx
 # Description		: 출고 > 기타출고 > 외부센터매각출고처리 (기타출고 결재)
 # Author					: JiHoPark
 # Since					: 2025.09.11.
 ############################################################################
*/
...
// API
import { apiSaveElectApproval, apiSaveMasterList3 } from '@/api/wd/apiWdShipmentETCEXDC';
...
interface WdShipmentETCEXDCDetail3Props {
	data: any;
	totalCnt: any;
	fixdccode: string;
	form: any;
...
const WdShipmentETCEXDCDetail3 = forwardRef((props: WdShipmentETCEXDCDetail3Props, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const gridCol = [
		...
		{ headerText: t('lbl.TYPE'), /*유형*/ dataField: 'inquirytypename', dataType: 'code', editable: false, width: 110 },
		{
			headerText: t('lbl.COSTCENTERNAME'),
			/*귀속부서명*/ dataField: 'costcentername',
			dataType: 'string',
			editable: false,
			width: 220,
		},
		...
		{
			headerText: t('lbl.COMP_PARTY_TYPE'),
			/*보상주체*/ dataField: 'compPartyType',
			dataType: 'string',
			width: 140,
			editable: true,
			renderer: {
				type: 'DropDownListRenderer',
				list: getCommonCodeList('COMP_PARTY_TYPE', t('lbl.SELECT'), ''),
				keyField: 'comCd',
				valueField: 'cdNm',
			},
		},
		{
			headerText: t('lbl.DEPOSIT_AMOUNT'),
			/*입금액*/ dataField: 'depositAmount',
			dataType: 'numeric',
			formatString: '#,##0.###',
			editable: true,
			width: 150,
		...
		{
			headerText: t('lbl.DIFF_AMOUNT'),
			/*차이금액*/ dataField: 'wrbtr',
			dataType: 'numeric',
			formatString: '#,##0.###',
			editable: false,
			width: 150,
		...
		{
			headerText: t('lbl.RSNCD'),
			/*사유코드*/ dataField: 'reasoncode',
			dataType: 'string',
			visible: false,
		},
	...
	const initEvent = () => {
		 ...
		 * @returns {void} 최종 수정값
		 */
		ref.current?.bind('cellEditEnd', function (event: any) {
			const curDataField = event.dataField;
			const curRowIndex = event.rowIndex;

			if (curDataField === 'depositAmount') {
				const newValue = commUtil.isEmpty(event.value) ? 0 : Number(event.value);
				const curStockamt = Number(event.item.stockamt);

				// 차이금액 = 입금액 - 재고금액
				//ref.current.setCellValue(curRowIndex, 'wrbtr', curStockamt - newValue);
				ref.current.setCellValue(curRowIndex, 'wrbtr', newValue - curStockamt);
			}
		});
	};
	 ...
	 */
	const saveCallback = () => {
		const updateGridData = ref.current.getChangedData().filter((item: any) => item.rowStatus === 'U');
		const delGridData = ref.current.getChangedData().filter((item: any) => item.rowStatus === 'D');

		if (updateGridData.length < 1 && delGridData.length < 1) {
			showMessage({
		...
		showConfirm(null, t('msg.MSG_COM_VAL_207', [0, updateGridData.length, delGridData.length]), () => {
			const params = {
				avc_DCCODE: props.fixdccode,
				saveMasterList3: updateGridData,
				deleteMasterList3: delGridData,
			};
			apiSaveMasterList3(params).then(res => {
				if (res.statusCode === 0) {
					showMessage({
						content: t('msg.confirmSaved'), // 삭제되었습니다.
						modalType: 'info',
					});
	...
	const saveElectApprovalCallback = () => {
		...
		showConfirm(null, t('msg.MSG_COM_VAL_216'), () => {
			const params = {
				...
				avc_COMMAND: 'APPROVAL_SA',
				processtype: 'AJ_APPROVAL_SA',
				temptabletype: 'AJ',
			...
			apiSaveElectApproval(params).then(res => {
				if (res.statusCode === 0) {
					...
					openElectApproval(res.statusMessage); // 전자결재 호출
				}
			});
		});
	};
	...
	return (
		<>
			...
			<GridAutoHeight id="other-shipment-payment">
...
export default WdShipmentETCEXDCDetail3;
