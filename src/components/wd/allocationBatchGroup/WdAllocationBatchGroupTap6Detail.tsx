/*
 ############################################################################
 # FiledataField	: WdAllocationBatchGroupTap6Detail.tsx
...
//Lib
import { apiSaveTab6MasterList } from '@/api/wd/apiWdAllocationBatchGroup';
...
const WdAllocationBatchGroupTap6Detail = forwardRef((props: any, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const confirmPopup = (selectedRow: any) => {
		...
		// if (gridData.some((row: any) => !row.custkey)) {
		// 	return;
		// }

		// // PK validation
		// if (!ref.gridRef.current.validatePKGridData(['dccode', 'storerkey', 'custkey'])) {
		// 	return;
		// }

		// PK validation
		if (!commUtil.validatePKData(ref.gridRef, ['dccode', 'storerkey', 'custkey'])) {
	...
	const saveMasterList = async () => {
		...
		showConfirm(null, t('msg.MSG_COM_CFM_003'), () => {
			...
			apiSaveTab6MasterList(itemsToSave).then(res => {
				if (res.statusCode == 0) {
	...
	return (
		<>
			{/* 그리드 영역 */}
			<CustomForm>
				<AGrid style={{ marginTop: '15px', marginBottom: '15px' }}>
					<GridTopBtn gridBtn={gridBtn} gridTitle="분배예외처리거래처 목록" totalCnt={props.totalCnt} />
				</AGrid>
				<GridAutoHeight id="WdAllocationBatchGroupTap6Detail-grid">
...
export default WdAllocationBatchGroupTap6Detail;
