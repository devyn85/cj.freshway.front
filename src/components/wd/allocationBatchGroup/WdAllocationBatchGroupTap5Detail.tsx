/*
 ############################################################################
 # FiledataField	: WdAllocationBatchGroupTap5Detail.tsx
...
//Lib
import { apiSaveTab5MasterList } from '@/api/wd/apiWdAllocationBatchGroup';
...
const WdAllocationBatchGroupTap5Detail = forwardRef((props: any, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const confirmPopup = (selectedRow: any) => {
		if (popupType === 'sku') {
			...
			ref.gridRef.current.setCellValue(ref.gridRef.current.getSelectedIndex()[0], 'custkeyname', selectedRow[0].name);
		}

		modalRef.current.handlerClose();

		// const gridData = ref.gridRef.current.getGridData();
		// if (gridData.some((row: any) => !row.custkey || !row.sku)) {
		// 	return;
		// }

		// // PK validation
		// if (!ref.gridRef.current.validatePKGridData(['dccode', 'sku', 'custkey'])) {
		// 	return;
		// }

		// PK validation
		if (!commUtil.validatePKData(ref.gridRef, ['dccode', 'sku', 'custkey'])) {
			...
			ref.gridRef.current.setCellValue(ref.gridRef.current.getSelectedIndex()[0], 'custkey', '');
			ref.gridRef.current.setCellValue(ref.gridRef.current.getSelectedIndex()[0], 'custkeyname', '');
	...
	const saveMasterList = async () => {
		...
		showConfirm(null, t('msg.MSG_COM_CFM_003'), () => {
			...
			apiSaveTab5MasterList(itemsToSave).then(res => {
				if (res.statusCode == 0) {
	...
	const gridCol = [
		...
		{
			headerText: t('lbl.USEBYDATE_FREE_RT'), //소비기한 잔여율
	...
	useEffect(() => {
		...
		ref.gridRef?.current.bind('cellClick', (event: any) => {
			if (event.dataField === 'sku') {
				...
				if (
					ref.gridRef.current.getCellValue(event.rowIndex, 'rowStatus') === 'I' ||
					commUtil.isEmpty(event.item.custkey)
				) {
					setPopupType('cust');
		...
		ref.gridRef?.current.bind('cellEditBegin', function (event: any) {
			...
			if (
				...
				//		event.dataField != 'sku' &&
				event.dataField != 'usebydateFreeRt' &&
				event.dataField != 'rmk1' /*RMK로 하면 FOCUSOUT이 안됨*/ &&
		...
		<>
			...
			<CustomForm>
				<AGrid style={{ marginTop: '15px', marginBottom: '10px' }}>
					<GridTopBtn gridBtn={gridBtn} gridTitle="거래처상품 상미율 목록" totalCnt={props.totalCnt} />
				</AGrid>
				<GridAutoHeight id="customer-product-satisfaction-rate-grid">
...
export default WdAllocationBatchGroupTap5Detail;
