/*
 ############################################################################
 # FiledataField	: WdDeliveryLabelSNTap4Detail.tsx
 # Description		: 출고 > 출고작업 > 이력배송라벨출력 - 기준정보
 # Author			: 공두경
 # Since			: 2025.10.17
 ############################################################################
*/
...
const WdDeliveryLabelSNTap4Detail = ({ gridData1, gridRef1, form, searchMasterList }: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const gridCol1 = [
		...
		},
		{
			...
			editRenderer: {
				...
				list: getCommonCodeList('PRINTORDER3', t('lbl.SELECT'), ''),
				keyField: 'comCd',
				valueField: 'cdNm',
			},
			labelFunction: (rowIndex: any, columnIndex: any, value: any) => {
				const name = getCommonCodebyCd('PRINTORDER3', value)?.cdNm;
	...
	const saveMasterList = async () => {
		...
		const updatedItems = gridRef1.current.getChangedData(); // 수정된 것만(체크박스 제외)
	...
	return (
		<>
			<AGrid style={{ padding: '10px 0 ', marginBottom: 0 }}>
				<GridTopBtn gridTitle={t('lbl.LIST')} gridBtn={gridBtn} totalCnt={gridData1?.length} />
			</AGrid>
			<GridAutoHeight>
...
export default WdDeliveryLabelSNTap4Detail;
