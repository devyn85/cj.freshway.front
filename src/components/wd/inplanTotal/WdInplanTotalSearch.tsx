/*
 ############################################################################
 # FiledataField	: WdInplanTotalSearch.tsx
 # Description		: 출고진행현황 Search
 # Author			: 공두경
 # Since			: 25.05.16
 ############################################################################
*/
...
const WdInplanTotalSearch = forwardRef((props: any, parentRef: any) => {
	...
	return (
		<>
			...
			<li>
				<CmCarSearch form={form} name="carnoNm" code="carno" label="차량번호" />
			</li>
			<li>
				<InputText
			...
			<li>
				<SelectBox
					label={t('lbl.SERIALTYPENAME3')} //유통이력
					name="serialtype"
					placeholder="선택해주세요"
					options={getCommonCodeList('PRODUCT-DRTYN', '--- 전체 ---').filter(item => !['2', '3'].includes(item.comCd))}
					fieldNames={{ label: 'cdNm', value: 'comCd' }}
				/>
			</li>
		</>
