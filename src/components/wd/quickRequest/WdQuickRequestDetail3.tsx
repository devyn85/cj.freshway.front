/*
 ############################################################################
 # FiledataField	: WdQuickRequestDetail3.tsx
...
저장 - 접수처리	
	saveMasterListCenterRecept02 

 집하지 저장
	const saveDetailListDelivery02 
 저장 - 도착지
	 ->saveDetailListDestination02
	외부 주문접수 API 호출
		saveMasterListOrderRequest01
...
// Utils
import CmZipPopup from '@/components/cm/popup/CmZipPopup';
...
const WdQuickRequestDetail3 = forwardRef<any, any>((props: any, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	 ...
	 * @param e
	 */
	const tabClickDetail1 = (key: string) => {
	 ...
	 * @returns
	 */
	const addrow01 = async () => {
		const gridRef = ref.gridRef.current;

		// 신규 행(rowStatus === 'I')이 이미 있으면 추가 불가
		const hasPendingNew = (gridRef?.getGridData() || []).some((row: any) => row.rowStatus === 'I');
		if (hasPendingNew) {
			showAlert(null, '신규 입력 중인 행이 있습니다. 저장 또는 삭제 후 추가하세요.');
			return;
		}
		...
		// 센터 또는 주소 검증
		const center = formRef2.getFieldValue('center');
		const address1 = formRef2.getFieldValue('address1');
		const address2 = formRef2.getFieldValue('address2');
		const zonecode = formRef2.getFieldValue('zonecode');

		const address = address1 + ' ' + address2;
		const gthAddrFull = zonecode + '||' + address1 + ' ' + address2;
		...
		// gridRef1의 기존 데이터 가져오기
		const existingData = gridRef?.getGridData() || [];
		...
		const newRow = {
			statusnm: '퀵주문등록', // 기본값
			rcptNo: '',
			rcptDate: dayjs().format('YYYY-MM-DD'), // 오늘 날짜
			reqId: user.userId, // 요청자
			reqNm: user.userNm,
			reqDepartment: user.department,
		...
		// 목록 총 개수 업데이트
		props.setTotalCnt(updatedData.length);
	...
	// 	// 7. 상세목록 총 개수 업데이트
	// 	props.setTotalCnt(newData.length);
	 ...
	 * @returns
	 */
	const addrow02 = async () => {
		const gridRef = ref.gridRef.current;
		const gridRef1 = ref.gridRef1.current;
		const gridRef2 = ref?.gridRef2?.current; // 안보이는Tab은 아직 로드가 안되어 있어 ? 처리
		...
		const newRow = {
			// gthSeq: 0, // 임시, 아래서 재정렬
			gthSeq: existingData.length > 0 ? Math.max(...existingData.map((row: any) => Number(row.gthSeq) || 0)) + 1 : 1, // 기존 그리드에서 max+1
			gthCd: radioValue === '1' ? center : '',
			gthNm: gthNm,
			gthAddr: radioValue === '1' ? centerInfo?.address || '' : address,
			gthAddrFull: radioValue === '1' ? centerInfo?.address || '' : gthAddrFull,
	 ...
	 * 저장 - 접수마스터
	 */
	const saveMasterListCenterRecept02 = async () => {
		const gridRef = ref.gridRef.current;
		const selectedRow = ref.gridRef.current?.getSelectedRows?.();
		const checkedItems = ref.gridRef.current?.getCheckedRowItems?.();
		const checkedRow = checkedItems[0].item; // 체크된 첫 번째 행
		...
		if (!isGridChangedData(ref.gridRef.current) && removedItems.length === 0 && removedItems.length === 0) {
		...
		showConfirm(null, t('msg.confirmSave'), () => {
			...
			const params = {
				...
				...pickGridFields(checkedRow, gridCol), // 마스터 정보 - 그리드 dataField만 추출
				...formRef4Values, // 귀속정보-하단
			};
			apiPostSaveMasterListCenterRecept02(params).then((res: any) => {
				if (res.statusCode === 0) {
	 ...
	 * 저장 - 집하지
	 */
	const saveDetailListDelivery02 = async () => {
		const gridRef = ref.gridRef1.current;
		...
		// 신규 행(rowStatus === 'I')이 이미 있으면 추가 불가
		const hasPendingNew = (ref.gridRef.current?.getGridData() || []).some((row: any) => row.rowStatus === 'I');
		if (hasPendingNew) {
			showAlert(null, '신규 입력 중인 행이 있습니다. 저장 또는 삭제 후 추가하세요.');
			return;
		}
		...
		if (!isGridChangedData(ref.gridRef1.current) && !formRef4Changed) {
			showAlert(null, t('msg.noChange')); // 변경사항이 없습니다.
			return;
		}
		...
		// gthSeq 1 미만 체크
		const zeroSeqRow = gridData.find((row: any) => Number(row.gthSeq) < 1);
		if (zeroSeqRow) {
			showAlert(null, '집하지순서 값이 1 미만인 행이 있습니다. 1 이상의 값을 입력해주세요.');
			return;
		}
		...
		gridData.forEach((row: any) => {
			const seq = Number(row.gthSeq);
			if (!isNaN(seq) && seq !== null && seq !== undefined) {
		...
		showConfirm(null, t('msg.confirmSave'), () => {
			const formRef4Values = formRef4 && typeof formRef4.getFieldsValue === 'function' ? formRef4.getFieldsValue() : {};
			const params = {
				avc_COMMAND: 'DELIVERY_COLLECTION_DIRECT',
				saveList02: saveList, // 선택된 행의 데이터
				fixdccode: form.getFieldValue('fixdccode') ?? '',
				...pickGridFields(selectedRow[0], gridCol), // 마스터 정보 - 그리드 dataField만 추출
				...formRef4Values, // 귀속정보
			};

			apiPostsaveDetailListDelivery02(params).then(res => {
				if (res.statusCode === 0) {
					showAlert(null, t('msg.save1')); // 저장되었습니다
					props.search();
					//searchDetail01List();
	 ...
	 * 저장 - 도착지
	 */
	const saveDetailListDestination02 = async () => {
		...
		showConfirm(null, t('msg.confirmSave'), () => {
			...
			// 집하지처리 저장 API 호출
			apiPostsaveDetailListDestination02(params).then(res => {
				if (res.statusCode === 0) {
					showAlert(null, t('msg.save1')); // 저장되었습니다
					//props.search();
					searchDetail02List();
	...
	// vocno/vsrno 중복 체크 함수
	/**
	 *
	 * @param data
	 */
	function hasDuplicateVocnoVsrno(data: any[]): boolean {
		const docKeyCount: { [key: string]: number } = {};
		data.forEach((row: any) => {
			const vocno = row.vocno ?? '';
			const vsrno = row.vsrno ?? '';
			const key = `${vocno}__${vsrno}`;
			if (vocno !== '' && vsrno !== '') {
				docKeyCount[key] = (docKeyCount[key] || 0) + 1;
			}
		});
		const dupDocKeys = Object.keys(docKeyCount).filter(key => docKeyCount[key] > 1);
		if (dupDocKeys.length > 0) {
			const msg = dupDocKeys
				.map(k => {
					const [vocno, vsrno] = k.split('__');
					return `VOC번호:${vocno}, VSR번호:${vsrno}`;
				})
				.join(' / ');
			showAlert(null, `VOC번호/VSR번호 값이 중복된 행이 있습니다.${msg}`);
			return true;
		}
		return false;
	}
	...
	const gridCol = [
		...
		//{ dataField: 'vocQty', headerText: t('수량'), width: 50, editable: false, dataType: 'numeric' }, // VOC량
		...
		// 추가 Select 컬럼들
		{
			dataField: 'deliverytype', // DELIVERYTYPE
			headerText: t('배송방법'),
			width: 120,
			dataType: 'code',
			editable: true,
		...
		{
			dataField: 'deliveryMethod', // DELIVERY_METHOD
			headerText: t('배송수단'),
			width: 120,
			dataType: 'code',
			required: true,
		...
		{
			dataField: 'articleType', // ARTICLE_TYPE
			headerText: t('물품종류'),
			width: 120,
			dataType: 'code',
			required: true,
		...
		{
			dataField: 'reservedate1',
			headerText: t('예약일'), // 예약일
			dataType: 'date',
			editable: true,
			width: 120,
		...
		{
			dataField: 'reservedate2',
			headerText: t('예약시분'), // 예약시분
			width: 120,
			dataType: 'code',
			editable: true,
			commRenderer: {
				type: 'dropDown',
				list: buildReserveTimeOptions(),
				keyField: 'comCd',
				valueField: 'cdNm',
				disabledFunction: function (rowIndex: any, columnIndex: any, value: any, item: any, dataField: any) {
					return isDisabled(item);
		...
		{ dataField: 'reservedate', editable: false, visible: islVisibleCol }, // reservedate
		//
	...
	const gridProps = {
		...
		isLegacyRemove: true, // 삭제행 추적 활성화
	...
	const saveMasterListReceipt = async () => {
		await saveMasterListOrderRequest01();
	};
	...
	const gridBtn: GridBtnPropsType = {
		...
		btnArr: [
			...
			},
			{
				...
				authType: 'save', // 권한
				callBackFn: saveMasterListReceipt,
			},
		],
	};
	 ...
	 * @param params1
	 */
	const saveMasterListOrderRequest01 = async (): Promise<any> => {
		const gridRef = ref.gridRef.current;
	...
	const gridBtn1: GridBtnPropsType = {
		...
		btnArr: [
			...
			},
			{
				...
				authType: 'save', // 권한
				callBackFn: deleteRows02,
			},
			{
				btnType: 'btn1' as const, // 저장
				btnLabel: t('lbl.SAVE'), // 저장
				authType: 'save', // 권한
				callBackFn: saveDetailListDelivery02, // 집하지 저장
			},
		],
	};
	...
	const gridBtn2: GridBtnPropsType = {
		...
		btnArr: [
			...
			// },
			{
				...
				btnLabel: t('lbl.SAVE'), // 저장
				authType: 'save', // 권한
				callBackFn: saveDetailListDestination02,
			},
		],
	};
	...
	// 우편번호 팝업 호출
	const cmZipPopup = (item: any, row: number): void => {
		setParams(item);
		setParams({ ...item, rowIndex: row });
		callPopModal01.current?.handlerOpen();
	};

	// 선택적용: formRef2(센터/주소) 값을 메인 그리드 체크 행 1개에만 반영
	const handleSelectApply = async (_flag?: string) => {
		const gridRef = ref.gridRef1.current;
		const checkedItems = gridRef.getCheckedRowItems();
		...
		if (!checkedItems || checkedItems.length === 0) {
			showAlert(null, t('msg.noSelect')); // '선택된 항목이 없습니다.'
			return;
		}
		...
		const newData = allData.map((row: any, index: number) => {
			if (checkedRowIndexes.has(index)) {
				return {
					...
					gthCd: radioValue === '1' ? center : row.gthCd,
					gthNm: gthNm == '' ? row.gthNm : gthNm,
					gthAddr: radioValue === '1' ? centerInfo?.address || '' : address,
					gthAddrFull: radioValue === '1' ? centerInfo?.address || '' : gthAddrFull,
					gthEmpNm: radioValue === '1' ? centerInfo?.empname1 || '' : '',
					gthTel: radioValue === '1' ? centerInfo?.empphone1 || '' : '', // 집하지연락처
					location: radioValue === '1' ? centerInfo?.location || '' : '',
					rowStatus: 'U', // 수정 행 상태로 설정
				};
			}
			return row;
		});
	...
	const tabItems = [
		{
			...
			children: (
				<>
					<AGrid style={{ height: 'auto', paddingTop: 10 }}>
						<GridTopBtn gridBtn={gridBtn1} gridTitle={t('집하지 목록')} totalCnt={detailTotalCnt1}>
							<CustomListForm form={formRef2} layout="inline">
								...
								{
									<li>
										<Button onClick={() => handleSelectApply('1')}>{t('선택적용')}</Button>
									</li>
								}
								{/* END.일괄적용영역 */}
							</CustomListForm>
		...
		},
		{
			...
			children: (
				<>
					<AGrid style={{ height: 'auto', paddingTop: 10, marginBottom: 0 }}>
		...
		<>
			<Splitter
				...
				items={[
					...
					<TabsArray
						key="wdQuickRequest-detail3-tab"
						activeKey={activeKeyDetail}
						onChange={tabClickDetail1}
						items={tabItems}
						onResizing={resizeAllGrids}
						onResizeEnd={resizeAllGrids}
					/>,
				...
				<Form
					...
					onValuesChange={changedValues => {
						...
						if (changedValues.respDept === '49') {
							showAlert(null, '고객사귀책(착불) 선택으로 인해 신용거래가 착불로 변경되었습니다.');
							// 고객사귀책(착불) 일경우 지급구분 자동 변경
							formRef.setFieldValue('payType', '2'); //  2:착불
						}
					}}
					style={{ padding: '10px 0', marginBottom: 0 }}
				>
					<UiDetailViewGroup ref={groupRef3}>
						...
						<li>
							<SelectBox
								name="respReason"
								label={t('귀책사유')} // 귀책사유
								options={getCommonCodeList('RESP_REASON_QUICK', '')}
								fieldNames={{ label: 'cdNm', value: 'comCd' }}
								required
								rules={[{ required: true, validateTrigger: 'none' }]}
								placeholder="선택해주세요"
			...
			<CustomModal ref={callPopModal01} width="500px">
				<CmZipPopup form={formRef2} close={callClosePopPopup01} param={params} callback={handleAddressCallback} />
			</CustomModal>
		</>
	);
});
export default WdQuickRequestDetail3;
