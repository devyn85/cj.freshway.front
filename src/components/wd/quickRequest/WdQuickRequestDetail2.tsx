...
const WdQuickRequestDetail2 = forwardRef((props: any, ref: any) => {
	...
	const handleSelectApply = async (flag: string) => {
		...
		if (filteredData.length < updatedData.length) {
			...
			showAlert(null, `VOC번호/VSR번호/주문번호가 중복된 ${removedCount}개 행이 제외되었습니다.`);
		}
	...
	function removeDuplicateVocnoVsrno(data: any[]): any[] {
		...
		return data.filter((row: any) => {
			...
			const key = `${vocno}__${vsrno}__${docno}`;

			// vocno, vsrno, docno가 모두 있는 경우에만 중복 체크
			if (vocno !== '' && vsrno !== '' && docno !== '') {
				if (seenKeys.has(key)) {
					return false; // 중복 제거
				}
				seenKeys.add(key);
	...
	const saveMasterListOrderRequest01Imp = async (): Promise<any> => {
		...
		if (activeKeyDetail === '1') {
			if (!ref.gridRef1?.current?.getGridData()?.length) {
	...
	return (
		<>
			...
			{/* 배송메모 */}
			<CustomModal ref={deliveryMemoModalRef} width="600px">
