/*
 ############################################################################
 # FiledataField	: WdAllocationCancelTap1Detail.tsx
 # Description		: 자동취소(Detail)
 # Author			: 공두경
 # Since			: 25.07.24
 ############################################################################
*/
...
// Utils
// API Call Function
import { apiGetDetailList, apiSaveAutoBatch } from '@/api/wd/apiWdAllocationCancel';
import GridAutoHeight from '@/components/common/GridAutoHeight';
import Splitter from '@/components/common/Splitter';

const WdAllocationCancelTap1Detail = forwardRef((props: any, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const onClickBatch = () => {
		...
		showConfirm(null, t('msg.MSG_COM_CFM_003', []), () => {
			const params = {
				avc_COMMAND: 'BATCHCANCEL_CUST',
				saveAutoBatchList: checkedRows, // 선택된 행의 데이터
			};

			apiSaveAutoBatch(params).then(res => {
				if (res.statusCode > -1) {
	...
	const gridCol = [
		...
		{ headerText: '관리처코드', dataField: 'custkey', dataType: 'code' },
		{ headerText: '관리처명', dataField: 'custname' },
				...
				items={[
					<>
						<AGrid style={{ padding: '10px 0', marginBottom: 0 }}>
							<GridTopBtn gridBtn={gridBtn} gridTitle="자동취소목록" totalCnt={props.totalCnt} />
						</AGrid>
						<GridAutoHeight>
					...
					<>
						<AGrid style={{ padding: '10px 0', marginBottom: 0 }}>
							<GridTopBtn gridBtn={gridBtn2} gridTitle="자동취소상세" />
						</AGrid>
						<GridAutoHeight>
...
export default WdAllocationCancelTap1Detail;
