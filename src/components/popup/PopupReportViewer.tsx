...
const PopupReportViewer = forwardRef<HTMLDivElement, ReportViewerProps>((props, ref) => {
	...
	useEffect(() => {
		if (window.m2soft && window.m2soft.crownix && window.m2soft.crownix.Viewer) {
			...
			// 1. 업무 화면에서 던진 DataSet
			const dataSet =
				props.dataSet && typeof props.dataSet === 'object' && !Array.isArray(props.dataSet) ? props.dataSet : {};
			// 2. 업무 화면에서 던진 파라미터
			...
			// 예시 : INVOICE_TITLE1[납품서 (공급받는자용)] INVOICE_TITLE2[납품서 (공급받는자용)]
			let dbParam = '/rv';
			...
			viewer.openFile(
				props.fileName,
				params && params.trim().length > 0 ? dbParam + ' /rxmlreportopt [0]' : '/rxmlreportopt [0]',
			);
		} else {
		}
	}, []);
