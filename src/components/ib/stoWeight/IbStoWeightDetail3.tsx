/*
 ############################################################################
 # FiledataField	: IbStoWeightDetail3.tsx
...
const IbStoWeightDetail3 = ({ gridRef, gridData, openModal, saveBatchMasterList }: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const gridCol2 = [
		...
		{
			dataField: 'exPartnerConfirmweight',
			headerText: t('제외협력사'),
			dataType: 'numeric',
			formatString: '#,##0.###',
			editable: false,
	...
	useEffect(() => {
		if (gridRef.current) {
			...
			gridData.forEach((row: any) => {
				...
				month.exPartnerConfirmweight += toNumberSafe(row.exPartnerConfirmweight);
			...
			if (gridData.length > 0) {
				...
				// yyyymmdd 칼럼 인덱스 찾기
				const yyyymmddIndex = gridCol2.findIndex((col: any) => col.dataField === 'yyyymmdd');
				if (yyyymmddIndex !== -1) {
	...
	return (
		<>
			<AGrid className="h100">
				...
				<AUIGrid ref={gridRef} columnLayout={gridCol2} gridProps={gridProps} />
			</AGrid>
		</>
	);
};
export default IbStoWeightDetail3;
