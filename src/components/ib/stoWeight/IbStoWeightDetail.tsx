/*
 ############################################################################
 # FiledataField	: IbStoWeightDetail.tsx
...
const IbStoWeightDetail = ({ gridRef, gridData, openModal, saveBatchMasterList }: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const gridCol = [
		...
		{
			dataField: 'dcWdConfirmweight',
			headerText: t('센터 총량출고(CJL)'),
			dataType: 'numeric',
			formatString: '#,##0.###',
			editable: false,
		},
		{
			dataField: 'dcWdJejuConfirmweight',
			headerText: t('센터 총량출고(제주)'),
			dataType: 'numeric',
			formatString: '#,##0.###',
			editable: false,
		},
	...
	useEffect(() => {
		if (gridRef.current) {
			...
			if (gridData.length > 0) {
				...
				// yyyymmdd 칼럼 인덱스 찾기
				const yyyymmddIndex = gridCol.findIndex((col: any) => col.dataField === 'yyyymmdd');
				if (yyyymmddIndex !== -1) {
...
export default IbStoWeightDetail;
