/*
 ############################################################################
 # FiledataField	: PagingDetailSampleDetail.tsx
...
interface PagingDetailDetaillProps {
	gridData: any;
	totalCount: any;
	setCurrentPage: any;
	searchDtl: any;
...
const PagingDetailSampleDetail = forwardRef((props: PagingDetailDetaillProps, gridRef: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const getGridCol = () => {
		const gridCol = [
			{
				dataField: 'ATTACH_YN',
				headerText: t('comfunc.bbs.grid.attchYn'), // 첨부파일
				width: 50,
				editable: false,
				commRenderer: {
					type: 'search',
	...
	const saveGrid = () => {
		...
		if (
			...
			!gridRef.current.validateGridData(['invoiceCrtType'])
			//!gridRef.current.validateChangedGridData(['invoiceCrtType'])
		) {
	...
	const saveDeatilInfo = async () => {
		...
		// 저장 실행
		props.saveDetailInfo([form.getFieldsValue(true)]);
	};
	 ...
	 */
	useEffect(() => {
		if (gridRef?.current && props.gridData) {
			gridRef.current.appendData(props.gridData);
	...
	return (
		<>
			...
			<CustomModal ref={refModal} width="1084px">
				<PopupFileUploader ref={refModal} paramAttchFileGrpNo={attchFileGrpNo} callBack={popupCallBack} />
			</CustomModal>

			{/* 상세 영역 정의, 밸리데이션을 위해서 help, validateStatus를 각 커스텀 컴포넌에 구현하면 됨 */}
			<Row gutter={8}>
				<Col span={24}>
								...
								<UiDetailViewGroup>
									...
									<li>
										<InputText
											...
											required
											rules={[
												{ required: true, validateTrigger: 'false' },
												() => ({
													validator(_, value) {
...
export default PagingDetailSampleDetail;
