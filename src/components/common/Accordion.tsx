...
const Accordion = ({
	...
	}, [defaultExpanded, items, isControlled]);
...
const AccordionPanel = ({ expanded, children, onResizeEnd }: AccordionPanelProps) => {
	...
	return (
		<PanelWrapper $expanded={expanded} $height={rendered ? height : 0} onTransitionEnd={handleTransitionEnd}>
...
const Header = styled.button<{
	...
	gap: 12px;
	padding: 4px;
	padding: ${({ $expanded }) => ($expanded ? '4px 4px 0px 4px' : '4px ')};
	border: 0;
