/*
 ############################################################################
 # FiledataField	: OmAutoOrderSetupPopup.tsx
 # Description		:  저장품자동발주관리 팝업
...
const OmAutoOrderSetupPopup = (props: PropsType) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	const gridCol2 = [
		{
			...
			commRenderer: {
				type: 'dropDown',
				list: getCommonCodeList('CUSTORDERCLOSETYPE'),
				disabledFunction: (rowIndex: number, columnIndex: number, value: any, item: any, dataField: string) => {
					if (item.rowStatus !== 'I') {
						return true;
					}
	...
	const handleDeleteRow = async () => {
		// if (searchTypeNo !== 1) {
		// 	showMessage({ content: '해당 상세설정에서만 삭제 가능합니다.' });
		// 	return;
		// }

		const grid = gridRefs[searchTypeNo].current;

		let selectedItems = grid.getCheckedRowItems?.() || [];

		selectedItems = selectedItems.filter((item: any) => item && item.item);

		if (selectedItems.length === 0) {
			showMessage({ content: '삭제할 행을 선택하세요.' });
			return;
		}
		// db에서 조회한 데이터
		const dbRows = selectedItems.filter((item: any) => item.item?.rowStatus !== 'I');

		// DB 삭제 대상만 API 파라미터 생성
		const deleteParams = dbRows.map((item: any) => ({
			...item.item,
			rowStatus: 'D',
			purchaseConfCd: purchaseConfCd,
			purchaseCd: gridData[0]?.purchaseCd,
		...
		showConfirm(null, '선택한 데이터를 삭제하시겠습니까?', async () => {
			try {
				// DB 데이터 있을 때만 API 호출
				if (deleteParams.length > 0) {
					await apiDeleteCloseType(deleteParams);
				}

				// 신규,기존 모두 화면에서 제거
				const sortedItems = [...selectedItems].sort((a: any, b: any) => b.rowIndex - a.rowIndex);

				sortedItems.forEach((item: any) => {
					grid.removeRow(item.rowIndex);
				});
			...
			{popupType === 'detail' && (
				<li>
					<SelectBox //속성
						...
						label={'상세 설정 명칭'}
						span={24}
						options={getCommonCodeList('AUTO_ORD_DETAIL_SET', t('lbl.SELECT'), null)}
...
export default OmAutoOrderSetupPopup;
