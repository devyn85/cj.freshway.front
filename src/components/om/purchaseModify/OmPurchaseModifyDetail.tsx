...
const OmPurchaseModifyDetail = forwardRef((props: any, ref: any) => {
	...
	const gridCol = [
		...
		{ headerText: '입고센터분배여부', dataField: 'allocationYn', dataType: 'code', visible: false }, // 입고센터분배여부(숨김)
	];
	...
	const deletePurchaseModifyList = () => {
		...
		if (props.purchaseType === 'PO') {
			...
			if (delList.some((row: any) => row.allocationYn === 'Y')) {
				showAlert(null, '입고센터에 분배중인 건이 있습니다.\n분배취소 후 삭제 가능합니다.');
				return;
			}
