...
const OmOrderCreationSTOOrdBaseFOSearch = (props: OmOrderCreationSTOOrdBaseFOSearchProps) => {
	...
	const prevTabKeyRef = useRef<string>('1');
	const prevTabKeyRef2 = useRef<string>('1');
	 ...
	 */
	useEffect(() => {
		if (props.currentTabKey === '1' && prevTabKeyRef2.current === '1') {
			props.callBack?.();
		}
	}, [fromDccodeWatch]);
	...
	useEffect(() => {
		prevTabKeyRef2.current = prevTabKeyRef.current;
		prevTabKeyRef.current = props.currentTabKey;
	}, [props.currentTabKey]);
