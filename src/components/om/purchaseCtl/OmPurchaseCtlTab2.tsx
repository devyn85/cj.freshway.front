...
// API Call Function
import { getTab2Detail2List, getTab2DetailList } from '@/api/om/apiOmPurchaseCtl';
...
const OmPurchaseCtlTab2 = forwardRef((props: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const setDetail2GridData = (params: any) => {
		// API 호출
		getTab2Detail2List(params).then(res => {
			props.detail2GridRef.current?.setGridData([]);
			if (res.data.length > 0) {
	...
	return (
		<Splitter
			...
			items={[
				<GridAutoHeight key="omPurchaseCtl-tab2-left-grid" style={{ paddingTop: 10 }}>
					<AUIGrid ref={props.gridRef} columnLayout={gridCol} gridProps={gridProps} footerLayout={masterFooterLayout} />
				</GridAutoHeight>,
				<Splitter
					direction="vertical"
					key="omPurchaseCtl-tab2-right-splitter"
					onResizing={resizeAllGrids}
					onResizeEnd={resizeAllGrids}
					items={[
						<GridAutoHeight key="omPurchaseCtl-tab2-right-grid-top" style={{ paddingTop: 10 }}>
							<AUIGrid
						...
						<GridAutoHeight key="omPurchaseCtl-tab2-right-grid-bottom" style={{ paddingTop: 10 }}>
							<AUIGrid
...
export default OmPurchaseCtlTab2;
