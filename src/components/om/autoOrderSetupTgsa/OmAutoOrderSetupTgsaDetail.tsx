/*
 ############################################################################
 # FiledataField	: OmAutoOrderSetupTgsaDetail.tsx
...
import OmAutoOrderSetupTgsaDetailTab from '@/components/om/autoOrderSetupTgsa/OmAutoOrderSetupTgsaDetailTab';
...
interface OmAutoOrderSetupTgsaDetailProps {
	form?: any;
...
const OmAutoOrderSetupTgsaDetail = forwardRef((props: OmAutoOrderSetupTgsaDetailProps, gridRef: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const searchMasterInfo = (params: any) => {
		apiGetMasterInfo(params).then(res => {
			...
			if (data) {
				const newMasterInfo: any = { ...data }; // 복사

				for (const key in data) {
					if (key.endsWith('Yn') && key !== 'delYn') {
						newMasterInfo[`${key}_checkbox`] = data[key] === '1';
					}
				}

				setMasterInfo(newMasterInfo); // 새 객체 넣기
			}
		});
	};
	...
	useEffect(() => {
		if (changeMasterInfo.status === 'I') {
			...
			const rowIndex = gridRef.current.getSelectedIndex()[0];
			if (rowIndex === undefined || rowIndex < 0) return;

			const origin = gridRef.current.getItemByRowIndex(rowIndex);

			gridRef.current.updateRow(rowIndex, {
				...origin,
				...changeMasterInfo,
			});
		}
		...
		<>
			<Splitter
				...
				items={[
					...
					<TabsArray key="OmAutoOrderSetupTgsaDetail-tabs" items={tabs} activeKey={activeKey} onChange={tabClick} />,
				]}
			/>
...
export default OmAutoOrderSetupTgsaDetail;
