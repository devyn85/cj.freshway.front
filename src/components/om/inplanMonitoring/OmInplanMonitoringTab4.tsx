...
// API Call Function
import { getChkFsDetailList, getOrderListChkFSIF } from '@/api/om/apiOmInplanMonitoring';
...
const OmInplanMonitoringTab4 = forwardRef((props: any, gridRef: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
		...
		props.gridRef?.current.bind('selectionConstraint', (event: any) => {
			...
			if (event.rowIndex !== event.toRowIndex) {
				...
				if (gridRefCur) {
					...
					// API 호출
					getChkFsDetailList(params).then(res => {
		...
		props.detailGridRef?.current.bind('selectionConstraint', (event: any) => {
			...
			if (event.rowIndex !== event.toRowIndex) {
				...
				if (gridRefCur) {
					...
					// API 호출
					getOrderListChkFSIF(params).then(res => {
	...
	const resizeAllGrids = useCallback(() => {
		...
		props.gridRef3?.current?.resize?.('100%', '100%');
	}, []);
	...
	return (
		<Splitter
			...
			items={[
				<GridAutoHeight key="omInplanMonitoring-tab4-grid" style={{ paddingTop: 10 }}>
					<AUIGrid ref={props.gridRef} columnLayout={gridCol} gridProps={gridProps} footerLayout={footerLayout} />
				</GridAutoHeight>,
				<Splitter
					key="omInplanMonitoring-tab4-bottom-splitter"
					onResizing={resizeAllGrids}
					onResizeEnd={resizeAllGrids}
					items={[
						<>
...
export default OmInplanMonitoringTab4;
