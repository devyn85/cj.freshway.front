/*
 ############################################################################
 # FiledataField	: TmAmountByCarExcelPopup.tsx
...
import {
	apiGetCarInfo,
	apiSaveExcel as apiSaveMasterList,
	apiGetExcelValChk as getExcelValChkv,
...
const TmAmountByCarExcelPopup = forwardRef((props: PropsType, ref: any) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	// 	const rowsToUse = (codeDtl || []).filter((r: any) => r?.rowStatus === 'I' && r?.valChk === 'Y');
	// 	// 수정:
	...
	const saveMaster = () => {
		...
		const rowsToUse = (codeDtl || []).filter((r: any) => r?.rowStatus === 'I' && r?.valChk === 'Y' && r.errYn === 'N');
	...
	const gridCol = [
		...
		// 	dataType: 'text',
		// 	labelFunction: (rowIndex: any, columnIndex: any, value: any) => {
		// 		return gridRef.current.getCellValue(rowIndex, 'couriername') || '';
		// 	},
		// },
		// {
				...
				conditionFunction: function (rowIndex, columnIndex, value, item) {
					if (item?.rowStatus === 'I') {
						return {
							...
							listFunction: function (rowIndex, columnIndex, item, dataField) {
								// getSttlItemList();
								// //console.log(('poplist == ', refSttlItemList.current);
	 ...
	 */
	useEffect(() => {
		refSttlItemList.current = props.codeList;
	}, [props.codeList]);
	useEffect(() => {
		...
		gridRef?.current.bind('cellEditEnd', (event: any) => {
			...
			if (event.dataField == 'sttlItemCd' || event.dataField === 'courier') {
				...
				// ref.gridRef.current.setCellValue(rowIndex, 'sttlItemType', data1);
				// //console.log((event);
	...
	useEffect(() => {
		if (gridRef) {
			const newData = (Array.isArray(gridData) ? gridData : [gridData]).map(item => ({
				...item,
				customRowCheckYn: item.errYn !== 'N' ? 'N' : 'Y',
				rowStatus: item.errYn === 'N' ? 'I' : '',
			}));
	...
	return (
		<>
			{/* 상단 타이틀 및 페이지버튼 */}
			<PopupMenuTitle name="차량별운송비관리엑셀업로드" showButtons={false} />
...
export default TmAmountByCarExcelPopup;
