/*
 ############################################################################
 # FiledataField	: StMKitDetail3.tsx
 # Description		: 재고 > 재고조정 > 전자결재[전자결재TAB]
 # Author		    	: 고혜미
 # Since			    : 25.11.04
 ############################################################################
*/
...
//API
import { apiPostSaveMasterList02, apiPostSaveMasterList05 } from '@/api/st/apiStMKit';
...
const StMKitDetail3 = forwardRef((props: any, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
			...
			children: [
				{
					...
					commRenderer: {
						...
						onClick: function (e: any) {
							...
							ref.gridRef3.current.openPopup(params, 'sku');
						},
					},
				}, // 상품코드
				{
	...
	const approvalMasterList = () => {
		...
		// 삭제 체크한 데이터가 있으면 확인한다.
		const delGridData = ref.current.getChangedData();
		if (delGridData.length > 0) {
			showMessage({
				content: t('msg.MSG_COM_VAL_214'),
				modalType: 'warning',
			});
			return;
		}

		// 결재 진행할 지 확인하고 결재 프로세스 처리한다.
		showConfirm(null, t('msg.MSG_COM_VAL_216'), () => {
			...
			// 저장 실행
			apiPostSaveMasterList02(params).then(res => {
				if (res.statusCode === 0) {
					// 전체 체크 해제
	...
	const saveMasterList = () => {
		...
		apiPostSaveMasterList05(params).then(res => {
			if (res.statusCode === 0) {
				showMessage({
	...
	// grid data 변경 감지
	useEffect(() => {
		const gridRef3Cur = ref.current;
		if (gridRef3Cur) {
			gridRef3Cur?.setGridData(props.data);
		}
	}, [props.data]);
