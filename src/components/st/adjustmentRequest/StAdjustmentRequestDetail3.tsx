/*
 ############################################################################
 # FiledataField	: StAdjustmentRequestDetail3.tsx
 # Description		: 재고 > 재고조정 > 재고조정처리 (재고조정 결재)
 # Author					: JiHoPark
 # Since					: 2025.10.16.
 ############################################################################
*/
...
// API
import { apiSaveElectApproval, apiSaveMasterList3 } from '@/api/st/apiStAdjustmentRequest';
...
interface StAdjustmentRequestDetail3Props {
	gDccode: string;
	data: any;
	totalCnt: any;
	searchHandler: any;
...
const StAdjustmentRequestDetail3 = forwardRef((props: StAdjustmentRequestDetail3Props, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	//그리드 컬럼
	const gridCol = [
		{ headerText: t('lbl.DCCODE'), /*물류센터*/ dataField: 'dccode', dataType: 'code', width: 80, editable: false },
		{ headerText: t('lbl.STORE'), /*창고*/ dataField: 'organize', dataType: 'code', width: 100, editable: false },
		{
			headerText: t('lbl.PLANORDER'),
			/*계획오더*/ dataField: 'planorder',
			dataType: 'code',
			editable: false,
			visible: false,
		},
		...
		{ headerText: t('lbl.REMARK_REASON'), /*비고(사유)*/ dataField: 'memo2', dataType: 'string', width: 180 },
		{
	...
	const initEvent = () => {
		...
		ref.current?.bind('cellEditEnd', function (event: any) {
			...
			if (curDataField === 'chk') {
				...
				const curPlanorder = ref.current.getCellValue(curRowIndex, 'planorder');

				if (commUtil.isNotEmpty(curPlanorder)) {
					if (newVal === '1') {
						ref.current?.addCheckedRowsByValue('planorder', curPlanorder);
					} else {
						ref.current?.addUncheckedRowsByValue('planorder', curPlanorder);
					}
				}
			}
		});
	...
	const saveCallback = () => {
		...
		if (delGridData.length < 1) {
			showMessage({
				content: t('msg.MSG_COM_VAL_007', ['삭제건']), // {{0}}을(를) 선택해 주십시오.
				modalType: 'info',
			});
			return;
		}
		...
		showConfirm(null, t('msg.MSG_COM_VAL_207', [0, 0, delGridData.length]), () => {
			const params = {
	...
	const openElectApproval = (xmlVal: string) => {
		...
		const params = {
			formSerial: props.gDccode === '2170' ? 'SCM08' : 'SCM06', // SCM01 -> SCM06, SCM03 -> SCM08
			systemID: 'SCM',
	...
	const saveElectApprovalCallback = () => {
		...
		const chkDataList = ref.current.getCustomCheckedRowItems();
		...
		const findData = chkDataList.find((item: any) => {
			if (commUtil.isEmpty(item.stockamt) || Number(item.stockamt) === 0 || commUtil.isNotEmpty(item.stockamtmsg)) {
				if (commUtil.isEmpty(item.stockamtmsg)) {
					errReason = '금액이 0인 건은 처리 불가능 합니다.';
				} else {
...
export default StAdjustmentRequestDetail3;
