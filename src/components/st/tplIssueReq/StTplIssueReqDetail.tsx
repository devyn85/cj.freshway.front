/*
 ############################################################################
 # FiledataField	: StTplIssueReqDetail.tsx
 # Description		: ​​정산항목관리
...
import { apisaveMasterList } from '@/api/st/apiStTplIssueReq';
...
import StTplIssueReqUploadExcelPopup from '@/components/st/tplIssueReq/StTplIssueReqUploadExcelPopup';
...
//store
const StTplIssueReqDetail = forwardRef((props: any, ref: any) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	const toCustkey = props.form.getFieldValue('custkey');
			...
			commRenderer: {
				...
				searchDropdownProps: {
					...
					callbackBeforeUpdateRow: (e: any) => {
						...
						setTimeout(() => {
							ref.gridRef?.current?.updateRow(updateObj, e.rowIndex);
		...
		btnArr: [
			{
				...
				callBackFn: () => {
					if (isEmpty(vendor) || isEmpty(organize) || isEmpty(tplBcnrId)) {
	...
	useEffect(() => {
		...
		gridRefCur.bind('cellEditEnd', async (event: any) => {
			...
			if (e === 'deliveryDate') {
				const slipDt = data.slipDt;

				if (data.deliveryDate < slipDt) {
					showAlert('', '출고일자는 입고일자보다 커야합니다.');
					gridRefCur.setCellValue(row, 'deliveryDate', event.oldValue);
					return;
				}
			}
		});
	...
	useEffect(() => {
		...
		if (gridRefCur) {
			...
			if (props.data.length > 0) {
				...
				gridRefCur.setColumnPropByDataField('durationFrom', { width: 93 });
				gridRefCur.setColumnPropByDataField('durationTo', { width: 93 });
				// gridRefCur.setColumnPropByDataField('sku', { width: 90 });
...
export default StTplIssueReqDetail;
