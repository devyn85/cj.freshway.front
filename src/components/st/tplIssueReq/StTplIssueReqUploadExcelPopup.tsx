/*
 ############################################################################
 # FiledataField	: StTplIssueReqUploadExcelPopup.tsx
...
import { apiGetExcelCheck, apisaveMasterList } from '@/api/st/apiStTplIssueReq';
import StTplReceiptReqFileUploadPopup from '@/components/comfunc/func/filePage/StTplReceiptReqFileUploadPopup';
import CustomModal from '@/components/common/custom/CustomModal';
import StTplReqSkuSearchPopup from '@/components/st/tplIssueReq/StTplReqSkuSearchPopup';
...
const StTplIssueReqUploadExcelPopup = forwardRef((props: PropsType, ref: any) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	const getDeliveryType_fs = (rowIndex: any, columnIndex: any, value: any) => {
		return getCommonCodebyCd('DELIVERYTYPE', value)?.cdNm;
	...
	const popupCallBack = (item: any) => {
		...
		if (!isEmpty(item.newDocNo)) {
			for (const e of codeDtl) {
				...
				// gridRef?.current?.setCellValue(, 'docNo', item.newDocNo);
			}
			...
			for (const e of list) {
				...
				gridRef.current.updateRowsById(data);
				// popupgridRef.current.addUncheckedRowsByValue('_$uid', e._$uid);
				// gridRef?.current?.setCellValue(, 'docNo', item.newDocNo);
			}
		}
	...
	const getSkuSelectData = (value: any, selectRow: number) => {
		...
		apiGetDataSelectSkuForMsExDcRate(param).then(res => {
			// const gridRef = gridRef.current;
			const data = res.data;
			if (!data) {
				gridRef.current.setCellValue(selectRow, 'storageTypeSku', '');
				gridRef.current.setCellValue(selectRow, 'netWeight', '');
				gridRef.current.setCellValue(selectRow, 'qtyPerBox', '');
				gridRef.current.setCellValue(selectRow, 'baseUom', '');
				gridRef.current.setCellValue(selectRow, 'specCode', '');
				gridRef.current.setCellValue(selectRow, 'skuName', '');
				gridRef.current.setCellValue(selectRow, 'boxPerPlt', '');
				gridRef.current.setCellValue(selectRow, 'layerPerPlt', '');
				gridRef.current.setCellValue(selectRow, 'durationType', '');
				gridRef.current.setCellValue(selectRow, 'orderQty', 0);
				gridRef.current.setCellValue(selectRow, 'boxQty', 0);
				gridRef.current.setCellValue(selectRow, 'pltQty', 0);

				return;
			}
			gridRef.current.setCellValue(selectRow, 'storageTypeSku', data.storageType);
			gridRef.current.setCellValue(selectRow, 'netWeight', data.netWeight);
			gridRef.current.setCellValue(selectRow, 'qtyPerBox', data.qtyPerBox);
			gridRef.current.setCellValue(selectRow, 'baseUom', data.baseUom);
			gridRef.current.setCellValue(selectRow, 'specCode', data.skuGroup);
			gridRef.current.setCellValue(selectRow, 'skuName', data.description);
			gridRef.current.setCellValue(selectRow, 'boxPerPlt', data.boxPerPlt);
			gridRef.current.setCellValue(selectRow, 'layerPerPlt', data.layerPerPlt);
			gridRef.current.setCellValue(selectRow, 'duration', data.duration);
			gridRef.current.setCellValue(selectRow, 'durationType', data.durationType);
			gridRef.current.setCellValue(selectRow, 'orderQty', 0);
			gridRef.current.setCellValue(selectRow, 'boxQty', 0);
			gridRef.current.setCellValue(selectRow, 'pltQty', 0);
		});
	...
	const gridCol = [
		...
		{ dataField: 'fromCustKey', headerText: 'fromCustKey', width: 120, editable: false, visible: false },
		{ dataField: 'toCustKey', headerText: 'toCustKey', width: 120, editable: false, visible: false },
		{ dataField: 'stockId', headerText: 'toCustKey', width: 120, editable: false, visible: false },
		// {
		// 	dataField: 'sku',
		// 	headerText: '상품코드',
		// 	width: 130,
		...
		// },
		{
			...
			required: true,
			datatype: 'code',
			labelFunction: getDeliveryType_fs,
			// renderer: {
			...
			editRenderer: {
				...
				conditionFunction: function (rowIndex, columnIndex, value, item) {
					if (item?.rowStatus === 'I') {
						return {
							type: 'DropDownListRenderer',
							list: getCommonCodeList('DELIVERYTYPE', '').filter(r => r.comCd === 'WOD' || r.comCd === 'WSD'),
							keyField: 'comCd', // key 에 해당되는 필드명
							valueField: 'cdNm',
						};
					}
		...
		},
		{
			...
			commRenderer: {
				...
				onClick: function (e: any) {
					...
					const data = gridRef.current.current.getSelectedRows();
		...
		{ dataField: 'skuName', headerText: '상품명', width: 160, editable: false, dataType: 'text' },
		{ dataField: 'baseUom', headerText: '단위', width: 80, editable: false, dataType: 'code' },
		{
			dataField: 'stockQty',
			headerText: '현재고',
			dataType: 'numeric',
		...
		// },
		{
			dataField: 'storageDate',
			headerText: '저장일자',
			dataType: 'code',
			width: 140,
			editable: false,
			labelFunction: function (rowIndex: any, columnIndex: any, value: any, headerText: any, item: any) {
				// slipDt: 입고일, deliveryDate: 출고(요청)일
				const slip = item?.slipDt;
				const delivery = item?.deliveryDate;
				if (!slip || !delivery) {
					return commUtil.isEmpty(value) || value === '' ? 'STD' : dayjs(value).format('YYYY-MM-DD') ?? '';
				}
				const dSlip = dayjs(slip);
				const dDelivery = dayjs(delivery);
				if (!dSlip.isValid() || !dDelivery.isValid()) {
					return commUtil.isEmpty(value) || value === '' ? 'STD' : dayjs(value).format('YYYY-MM-DD') ?? '';
				}
				const days = dDelivery.diff(dSlip, 'day') + 1;
				return `${days}일`;
			},
		},
		{
	...
	return (
		<>
			{/* 상단 타이틀 및 페이지버튼 */}
			<PopupMenuTitle name="위탁출고요청" showButtons={false} />
			...
			<StTplReqSkuSearchPopup ref={refModal} />
			{/* 엑셀 파일 업로드 INPUT 영역 */}
			<input
...
export default StTplIssueReqUploadExcelPopup;
