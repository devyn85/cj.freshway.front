/*
 ############################################################################
 # FiledataField	: StAdjustmentRequestExDCDetail1.tsx
 # Description		: 재고 > 재고조정 > 외부비축재고조정처리 (재고 조정 요청 목록)
 # Author					: JiHoPark
 # Since					: 2025.08.25.
 ############################################################################
*/
...
import StAdjustmentRequestExDCDetail1UploadExcelPopup from '@/components/st/adjustmentRequestExDC/StAdjustmentRequestExDCDetail1UploadExcelPopup';
...
// API
import { apiSaveMasterList1 } from '@/api/st/apiStAdjustmentRequeStExDC';
...
interface StAdjustmentRequestExDCDetail1Props {
	data: any;
	totalCnt: any;
	form: any;
	initialValues: any;
...
const StAdjustmentRequestExDCDetail1 = forwardRef((props: StAdjustmentRequestExDCDetail1Props, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const saveCallback = () => {
		...
		chkDataList.forEach((item: any) => {
			// 작업박스수량에 값이 존재하고 조정수량에는 값이 0 또는 빈값인 경우
			if (commUtil.isNotEmpty(item.tranbox) && (commUtil.isEmpty(item.tranqty) || Number(item.tranqty) === 0)) {
				boxSaveList.push(item);
			} else {
				saveList.push(item);
			}
		});

		const boxFindData = boxSaveList.find((data: any) => {
			...
			if (Number(curTranbox) === 0) {
				errMsg = t('msg.placeholder1', [t('lbl.TRANBOXQTY')]); // 작업박스수량을 입력해주세요.
				return true;
			}

			if (Number(curRealcfmbox) + Number(curTranbox) < 0) {
				errMsg = t('msg.MSG_COM_VAL_237', []); // 박스수량과 작업박스수량의 합은 0 이상이어야 합니다.
				return true;
			}
		...
		const findData = saveList.find((data: any) => {
			...
			if (Number(curTranqty) === 0 || commUtil.isEmpty(curTranqty)) {
				errMsg = t('msg.placeholder1', [t('lbl.ADJUSTQTY_PLU_MINU')]); // 조정수량을 입력해주세요.
				return true;
			}
			...
			const findReason = getCommonCodeList('REASONCODE_AJAJ').find(item => item.comCd === curReasoncode);
			if (findReason.data2 === 'Y') {
				wdInfoYn = 'Y';

				if (Number(data.tranqty) > 0) {
					errMsg = findReason.cdNm + t('msg.MSG_COM_VAL_231'); // 사유 선택시 감모(음수)만 처리 가능합니다.
					return true;
				}
			}
		});
		...
		if (wdInfoYn === 'Y') {
			if (commUtil.isEmpty(reqFormData.wdCust)) {
				showMessage({
					content: t('msg.MSG_COM_VAL_218', ['출고자 정보']), // {{0}} 을/를 입력하십시요.
					modalType: 'info',
				});
				return;
			}
			if (commUtil.isEmpty(reqFormData.wdMethod)) {
				showMessage({
					content: t('msg.MSG_COM_VAL_218', [t('lbl.DELIVERY_TYPE')]), // {{0}} 을/를 입력하십시요.
					modalType: 'info',
				});
				return;
			}
		...
		showConfirm(null, t('msg.MSG_COM_VAL_207', [0, chkDataList.length, 0]), () => {
			const params = {
				...
				boxProcedure: 'SPST_CONVERT',
				boxCommand: 'CONFIRM_AJ_BATCH',
				boxProcesstype: 'ST_ADJUSTMENTBATCH',
				saveBoxAdjustList: boxSaveList,
				...
				if (curDataField === 'tranqty') {
					if (commUtil.isEmpty(newVal) || newVal === 0) {
						...
						if (curBoxflag === 'Y' && curAvgweight > 0) {
							ref.current.setCellValue(curRowIndex, 'tranbox', Math.round(newVal / curAvgweight));
						}
	...
	const closeEvent = () => {
		...
		if (excelChkDataList.length > 0) {
			...
			const excelParams = excelChkDataList.map((item: any) => {
				...
				// 발생사유
				const findReason = getCommonCodeList('REASONCODE_AJAJ').find(val => {
	...
	return (
		<>
			...
			<GridAutoHeight id="external-warehouse-inventory-adjustment-grid">
			...
			<CustomModal ref={refModal2} width="1000px">
				<StAdjustmentRequestExDCDetail1UploadExcelPopup close={closeEvent} ref={excelRef} />
			</CustomModal>
		</>
	);
});

export default StAdjustmentRequestExDCDetail1;
