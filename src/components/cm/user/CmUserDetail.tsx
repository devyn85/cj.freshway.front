...
/**
 * UTF-8 Base64 디코딩 (Java의 Base64.getEncoder().encodeToString(value.getBytes()) 처리용)
...
const CmUserDetail = forwardRef((props: any, ref: any) => {
	...
	const searchUserDtl = () => {
		...
		if (selectedRow?.length > 0 && !ref.gridRefUser.current.isAddedById(selectedRow[0]?._$uid)) {
			...
			// 20260326#[개인정보 1건도 취약점에 보고되어 처리]사용자 이름, 핸드폰 번호, 이메일을 Base64로 인코딩하여 반환 by sss
			apiGetCmUserDetail(params).then(res => {
				const dataFromDB = {
					...res.data,
					userNm: res.data.userNm ? decodeUTF8Base64(res.data.userNm) : '',
					userNmDisp: res.data.userNmDisp ? decodeUTF8Base64(res.data.userNmDisp) : '',
					handphoneNo: res.data.handphoneNo ? decodeUTF8Base64(res.data.handphoneNo) : '',
					mailId: res.data.mailId ? decodeUTF8Base64(res.data.mailId) : '',
