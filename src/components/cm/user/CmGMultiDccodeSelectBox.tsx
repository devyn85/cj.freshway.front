 ...
 * @returns
 */
const CmGMultiDccodeSelectBox = (props: any) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	useEffect(() => {
		if (commUtil.isNotEmpty(globalVariable.gDccode)) {
			...
			// 고객요청으로 원복 20260319 공두경(아래소스는 빈값일때 무조건 글로벌센터코드로 채워넣기 때문에 문제가 발생함) - 지라 : 8309
			// const currentValue = form.getFieldValue(name);
			// if (commUtil.isEmpty(currentValue) && commUtil.isNotEmpty(props.initval)) {
			// 	form.setFieldValue(name, isSingleMode ? globalVariable.gDccode : [globalVariable.gDccode]);
			// }
		} else {
	...
	return (
		<SelectBox
			...
			showSearch={window.location.hostname === 'localhost'} // 개발환경에서만 검색 기능 활성화
			filterOption={
...
export default CmGMultiDccodeSelectBox;
