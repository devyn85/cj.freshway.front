/*
 ############################################################################
 # FiledataField	: CmTaxTypeSearch.tsx
...
// component
import CmTaxTypePopup from '@/components/cm/popup/CmTaxTypePopup';
import CustomModal from '@/components/common/custom/CustomModal';
import { InputSearch } from '@/components/common/custom/form';

// API Call Function

import { apiGetTaxTypeList } from '@/api/cm/apiCmSearch';
...
const CmTaxTypeSearch = (props: SearchProps) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	const getSearchApi = (isPopup: boolean, value: string) => {
		...
		apiGetTaxTypeList(params).then(res => {
			if (!isPopup) {
	...
	const settingSelectData = (val: any) => {
		...
		let searchData1 = val[0].data1;

		for (let i = 1; i < val.length; i++) {
			searchName += `,${commUtil.convertCodeWithName(val[i].code, val[i].name)}`;
			searchCode += ',' + val[i].code;
			searchData1 += ',' + val[i].data1;
		}

		if (returnValueFormat === 'code') {
			form.setFieldsValue({ [name]: searchCode, [code]: searchCode, [data1]: searchData1 });
		} else if (val.length > 1) {
			form.setFieldsValue({ [name]: `${val.length}건 선택`, [code]: searchCode, [data1]: searchData1 });
		} else {
			form.setFieldsValue({ [name]: searchName, [code]: searchCode, [data1]: searchData1 });
		}

		if (props.onConfirm) {
			props.onConfirm(searchData1);
		}
...
export default CmTaxTypeSearch;
