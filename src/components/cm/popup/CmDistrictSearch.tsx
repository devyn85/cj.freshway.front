/*
 ############################################################################
 # FiledataField	: CmDistrictSearch.tsx
...
// component
import CmDistrictPopup from '@/components/cm/popup/CmDistrictPopup';
import CustomModal from '@/components/common/custom/CustomModal';
import { InputSearch } from '@/components/common/custom/form';

// API Call Function
import { apiGetCmDistrictList } from '@/api/cm/apiCmSearch';
...
const CmDistrictSearch = (props: SearchProps) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	 */
	const { form, selectionMode, name, code, returnValueFormat, required, value } = props;
	...
	const getSearchApi = (isPopup: boolean, value: string) => {
		...
		apiGetCmDistrictList(params).then(res => {
			if (!isPopup) {
	...
	}, [currentPageScr, popupForm, name, searchScroll]);
	...
	return (
		<>
			...
			>
				<InputSearch
					placeholder={t('msg.placeholder1', ['배송권역코드 또는 이름'])}
					form={form}
					name={name}
					code={code}
					hidden={true}
...
export default CmDistrictSearch;
