...
// component
import CmCustPopup from '@/components/cm/popup/CmCustPopup';
...
const CmCustSearch = (props: SearchCustProps) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	const getSearchApi = (isPopup: boolean, value: any) => {
		...
		if (typeof value === 'object' && value !== null) {
			if (!value.name && !value.dlvSearchVal && !value.saleCustSearchVal && !value.childCustSearchVal) {
		...
		if (expandedColumns === 'Y') {
			...
			if (typeof value === 'object') {
				...
				params.dlvSearchVal = value.dlvSearchVal || '';
				params.saleCustSearchVal = value.saleCustSearchVal || '';
	...
	const searchScroll = throttle((isPopup: boolean, value1: string | any, value2?: string) => {
		...
		if (expandedColumns === 'Y') {
			...
			if (typeof value1 === 'object') {
				...
				params.dlvSearchVal = value1.dlvSearchVal || '';
				params.saleCustSearchVal = value1.saleCustSearchVal || '';
				...
				if (params.saleCustSearchVal === '' && params.multiSelect === '' && params.dlvSearchVal === '') {
					gridRef.current?.showColumnByDataField('saleCustKey');
					gridRef.current?.showColumnByDataField('saleName');
					gridRef.current?.showColumnByDataField('mngCustkey');
					gridRef.current?.showColumnByDataField('mngName');
				} else if (params.saleCustSearchVal === '' && params.multiSelect === '') {
					gridRef.current?.hideColumnByDataField('saleCustKey');
					gridRef.current?.hideColumnByDataField('saleName');
					gridRef.current?.hideColumnByDataField('mngCustkey');
					gridRef.current?.hideColumnByDataField('mngName');
		...
		if (currentPageScr > 1) {
			if (expandedColumns === 'Y') {
				...
				const searchParams = {
					...
					dlvSearchVal: popupForm.getFieldValue('dlvSearchVal'),
					saleCustSearchVal: popupForm.getFieldValue('saleCustSearchVal'),
	...
	return (
		<>
			...
			>
				<InputSearch
					placeholder={props.placeholder ?? t('msg.placeholder1', [`${label ? label : '고객코드'} 또는 이름`])}
					form={form}
					name={name}
					code={code}
					hidden={true}
