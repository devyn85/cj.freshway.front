...
// utils
import Constants from '@/util/constants'; // constants import 경로 수정

// component
import CmDcPopup from '@/components/cm/popup/CmDcPopup';
import CustomModal from '@/components/common/custom/CustomModal';
import { InputSearch } from '@/components/common/custom/form';

// API Call Function
import { apiGetDcList } from '@/api/cm/apiCmSearch';
...
const CmDcSearch = (props: SearchDcProps) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	 */
	const { form, label, selectionMode, name, code, returnValueFormat, value, required, rules } = props;
	...
	const [pageSizeScr] = useState(Constants.PAGE_INFO.PAGE_SIZE); // constants → Constants
	 ...
	 * @returns {void}
	 */
	const searchScroll = throttle((isPopup: boolean, value1: string, value2 = '') => {
		const tt = currentPageScr - 1;
		const params = {
			name: value1,
			multiSelect: value2,
	...
	const settingSelectData = (val: any) => {
		...
		if (returnValueFormat === 'code') {
			// 아래 readOnly 체크하기 위해 강제로 변경 로직 추가
	...
	return (
		<>
			...
			>
				<InputSearch
					label={label ?? '센터'}
					placeholder={t('msg.placeholder1', ['센터코드 또는 이름'])}
					form={form}
					name={name}
					code={code}
					hidden={true}
