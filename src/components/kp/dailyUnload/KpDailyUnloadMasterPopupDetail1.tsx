/*
 ############################################################################
 # FiledataField	: KpDailyUnloadMasterPopupDetail1.tsx
 # Description		:  지표 > 생산성 > 데일리 생산성 하역 지표 관리(투입인원) 마스터관리 팝업 > 센터업무관리
 # Author			: JiHoPark
 # Since			: 2026.01.22.
 ############################################################################
*/
...
interface KpDailyUnloadMasterPopupDetail1Props {
	data: any;
	totalCnt: number;
	dsDccode: any;
	dsDailyU1: any;
...
const KpDailyUnloadMasterPopupDetail1 = forwardRef((props: KpDailyUnloadMasterPopupDetail1Props, ref: any) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	 */
	const { data, totalCnt, dsDccode, dsDailyU1, today, dayEndofYear } = props;
	...
	const gridCol = [
		...
		{ headerText: t('lbl.EXCPTYN'), /*예외여부*/ dataField: 'excptYn', dataType: 'code', visible: false },
	];
	...
	const initEvent = () => {
		...
		ref?.current.bind('cellEditBegin', function (event: any) {
			if (event.item.rowStatus !== 'I') {
				...
				if (curDataField !== 'contractFromdate' && curDataField !== 'gubun3') {
					return false;
				}
			}
		});
		...
		btnArr: [
			{
				...
				initValues: {
					...
					contractTodate: dayEndofYear.format('YYYYMMDD'), // 계약종료일
					gubun3: 0, // 인원(T/O)
				},
			},
			{
	...
	return (
		<AGrid>
			<GridTopBtn gridBtn={gridBtn} gridTitle={t('lbl.DCCODE_TASK_MNG')} totalCnt={totalCnt} />
			<AUIGrid ref={ref} columnLayout={gridCol} gridProps={gridProps} />
		</AGrid>
	);
});

export default KpDailyUnloadMasterPopupDetail1;
