/*
 ############################################################################
 # FiledataField	: KpDailyUnloadDatail1UploadExcelPopup.tsx
...
interface KpDailyUnloadDatail1UploadExcelPopupProps {
	close?: any;
}

const KpDailyUnloadDatail1UploadExcelPopup = forwardRef(
	(props: KpDailyUnloadDatail1UploadExcelPopupProps, ref: any) => {
		/**
		 * =====================================================================
		 *	01. 변수 선언부
		 * =====================================================================
		...
		const verificationCallback = () => {
			...
			curGridDataList.forEach((item: any) => {
				...
				if (commUtil.isEmpty(item.tranqty) || Number(item.tranqty) === 0) {
					// 조정수량
					verMsg += t('msg.placeholder1', [t('lbl.ADJUSTQTY_AJ')]);
				}

				if (commUtil.isEmpty(item.reasoncode)) {
					// 발생사유
					verMsg += t('msg.placeholder1', [t('lbl.INQUIRYREASONCODE')]);
				}

				if (commUtil.isEmpty(item.costcd)) {
					// 귀속부서
					verMsg += t('msg.placeholder1', [t('lbl.COSTCENTER')]);
				}

				if (commUtil.isEmpty(item.custkey)) {
					// 거래처
					verMsg += t('msg.placeholder1', [t('lbl.CUST')]);
				}
				...
				if (commUtil.isEmpty(item.lot)) {
					// LOT
					verMsg += t('msg.placeholder1', [t('lbl.TO_LOT')]);
				}

				if (commUtil.isEmpty(item.stockid)) {
					// STOCKID
					verMsg += t('msg.placeholder1', [t('lbl.STOCKID')]);
				}
				...
				if (commUtil.isEmpty(item.stocktype)) {
					// 재고위치
					verMsg += t('msg.placeholder1', [t('lbl.STOCKTYPE')]);
				}
		...
		return (
			<>
				...
				<AGrid>
					<GridTopBtn gridBtn={getGridBtn()} gridTitle={''} />
...
export default KpDailyUnloadDatail1UploadExcelPopup;
