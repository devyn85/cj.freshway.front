/*
 ############################################################################
 # FiledataField	: KpKxCloseT06ExcelUpload.tsx
 # Description		:  KX마감진행 현황 - 조정요청내역 엑셀 업로드
 # Author			: ParkYosep
 # Since			: 25.11.27
 ############################################################################
*/
...
const KpKxCloseT06ExcelUpload = (props: PropsType) => {
		...
		btnArr: [
			...
			},
			{
				...
				callBackFn: () => {
					...
					const saveData = checkedItems.map(row => row.item);
	...
	return (
		<>
			{/* 상단 타이틀 및 페이지버튼 */}
			<PopupMenuTitle
				name={t('lbl.ADJUSTQTY_ST') + t('lbl.REQUEST') + t('lbl.DESCRIPTION') + ' ' + t('lbl.EXCEL_UPLOAD')}
				showButtons={false}
			/>
