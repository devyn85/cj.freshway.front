...
import MsDeliveryDistrictPopPopup from '@/components/ms/popup/MsDeliveryDistrictPopPopup';
import { CloseCircleFilled } from '@ant-design/icons';

// api calls
import { apiPostGetPopList } from '@/api/ms/apiMsDeliveryDistrict';
...
interface IMsDeliveryDistrictPopSearchProps {
  ...
  name: string;           // 표시 필드명 (예: 'popName')
  code: string;           // 실제 값 필드명 (예: 'popno')
...
const MsDeliveryDistrictPopSearch = ({ form, selectionMode, name, code, returnValueFormat, label, required, value, disabled, customDccode }: IMsDeliveryDistrictPopSearchProps) => {
  ...
  const user = useAppSelector(s => s.user.userInfo);
	...
	const settingSelectData = (val: any): void => {
		let searchName = `[${val[0].popNo}]${val[0].popName || ''}`;
		let searchCode = val[0].popNo;

		for (let i = 1; i < val.length; i++) {
			searchName += `,[${val[i].popNo}]${val[i].popName || ''}`;
			searchCode += ',' + val[i].popNo;
		}
   ...
   */
	const getSearchApi = (isPopup: boolean, value: string, isMultiSelect: boolean) => {
      ...
      multiSelect: '',
		...
		apiPostGetPopList(params).then((res: any) => {
  ...
  */
  ...
  /**
	...
	const handleDropdownClick = (val: any) => {
		if (returnValueFormat === 'code') {
			form.setFieldsValue({ [name]: val.popNo, [code]: val.popNo });
		} else {
			form.setFieldsValue({ [name]: `[${val.popNo}]${val.popName || ''}`, [code]: val.popNo });
		}
					...
					<tbody>
						{dropdownData.map((item, index) => (
							<tr key={index} onClick={() => handleDropdownClick(item)}>
								<td id="dropdownTable">{item.popNo}</td>
								<td id="dropdownTable">{item.popName}</td>
							</tr>
						))}
					</tbody>
				</table>
  ...
  return (
    <>
      ...
      <Dropdown placement="bottom" open={dropdownOpen} trigger={[]} popupRender={dropdownRenderFormat}>
        <InputSearch 
          label={label ?? '대표 POP'}
          placeholder={t('msg.placeholder1', ['대표 POP 번호 또는 명'])}
          form={form}
          name={name}
          code={code}
          value={value}
