...
import MsDeliveryDistrictGroupPopup from '@/components/ms/popup/MsDeliveryDistrictGroupPopup';
...
interface IMsDeliveryDistrictGroupSearchProps {
  ...
  name: string;           // 표시 필드명 (예: 'dlvgroupNm')
  code: string;           // 실제 값 필드명 (예: 'dlvgroupId')
  returnValueFormat?: string; // 'code' - 코드만 반환
  label?: string;         // 라벨 (InputSearch 에서 사용)
  required?: any;         // 필수 여부 (InputSearch 에서 사용)
...
const MsDeliveryDistrictGroupSearch = ({ form, selectionMode, name, code, returnValueFormat, label, required, value, disabled, customDccode }: IMsDeliveryDistrictGroupSearchProps) => {
  ...
  const user = useAppSelector(s => s.user.userInfo);
	...
	const settingSelectData = (val: any): void => {
		let searchName = `[${val[0].dlvgroupId}]${val[0].dlvgroupNm || ''}`;
		let searchCode = val[0].dlvgroupId;

		for (let i = 1; i < val.length; i++) {
			searchName += `,[${val[i].dlvgroupId}]${val[i].dlvgroupNm || ''}`;
			searchCode += ',' + val[i].dlvgroupId;
		}
   ...
   */
	const getSearchApi = (isPopup: boolean, value: string, isMultiSelect: boolean) => {
      ...
      multiSelect: '',
		...
		apiPostGetDistrictGroupList(params).then((res: any) => {
  ...
  */
  ...
  /**
	...
	const handleDropdownClick = (val: any) => {
		if (returnValueFormat === 'code') {
			form.setFieldsValue({ [name]: val.dlvgroupId, [code]: val.dlvgroupId });
		} else {
			form.setFieldsValue({ [name]: `[${val.dlvgroupId}]${val.dlvgroupNm || ''}`, [code]: val.dlvgroupId });
		}
					...
					<tbody>
						{dropdownData.map((item, index) => (
							<tr key={index} onClick={() => handleDropdownClick(item)}>
								<td id="dropdownTable">{item.dlvgroupId}</td>
								<td id="dropdownTable">{item.dlvgroupNm}</td>
							</tr>
						))}
					</tbody>
				</table>
  ...
  return (
    <>
      ...
      <Dropdown placement="bottom" open={dropdownOpen} trigger={[]} popupRender={dropdownRenderFormat}>
        <InputSearch 
          label={label ?? '권역그룹'}
          placeholder={t('msg.placeholder1', ['권역그룹 ID 또는 명'])}
          form={form}
          name={name}
          code={code}
          value={value}
