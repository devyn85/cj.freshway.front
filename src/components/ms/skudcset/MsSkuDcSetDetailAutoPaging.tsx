...
const MsSkuDcSetDetailAutoPaging = forwardRef((props: MsSkuDcSetDetailProps, gridRef: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	useEffect(() => {
		if (gridRef?.current && props.data) {
			...
			// const newData = gridData.concat(data);
	...
	/**
	 * 조회 완료 감지되면 첫 행의 상세 정보를 조회한다.
	 */
	useEffect(() => {
		if (!props.isLoading) {
			gridRef?.current.setSelectionByIndex(0, 0);
			//searchDtl(gridRef?.current.getItemByRowIndex(0));
		}
	}, [props.isLoading]);
...
export default MsSkuDcSetDetailAutoPaging;
