/*
 ############################################################################
 # FiledataField	: MsSkuDcSetDetailButtonPaging.tsx
 # Description		: 센터상품속성
...
const MsSkuDcSetDetailButtonPaging = forwardRef((props: MsSkuDcSetDetailProps, gridRef: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const [gridTitle] = useState<string>(t('lbl.SKUDCSET_GRID_TITLE')); // 그리드 제목
	...
	// 	const curIdxPre = gridRef?.current.getSelectedIndex();
	// 	if (!commUtil.isEmpty(curIdxPre) && curIdxPre[0] !== 0) {
	// 		gridRef?.current.getItemByRowIndex(curIdxPre[0]);
	// 		searchDtl(gridRef?.current.getItemByRowIndex(curIdxPre[0]));
	// 	}
	// };
	...
	const setTableBtn = () => {
		const tableBtn: TableBtnPropsType = {
			...
			btnArr: [
				...
				{
					btnType: 'new', // 신규
					callBackFn: HandleAddDeatilInfo,
				},
				{
					btnType: 'save', // 저장
	...
	/**
	 * 데이터를 조회하면 그리드에 바인딩한다.
	 */
	useEffect(() => {
		if (gridRef?.current && props.data) {
			gridRef.current.setGridData(props.data);
	...
	return (
		<>
			{/* 그리드 영역 정의 */}
			<AGrid>
				<GridTopBtn gridTitle={gridTitle} gridBtn={getGridBtn()} totalCnt={props.totalCount} />
				...
				<Col span={8}>
					<SelectBox
						...
						options={[
							{ cdNm: '100', comCd: '100' },
							{ cdNm: '200', comCd: '200' },
							{ cdNm: '1000', comCd: '1000' },
							{ cdNm: '2000', comCd: '2000' },
						]}
						...
						<AGrid>
							...
							<UiDetailViewArea>
								<UiDetailViewGroup>
									...
									<li></li>
									<li>
										<InputText name="adddate" label={t('lbl.ADDDATE')} maxLength={20} allowClear disabled={true} />
									</li>
									<li>
										<InputText name="editdate" label={t('lbl.EDITDATE')} maxLength={20} allowClear disabled={true} />
									</li>
									<li>
										<InputText name="addwho" label={t('lbl.ADDWHO')} maxLength={20} allowClear disabled={true} />
									</li>
									<li>
										<InputText name="editwho" label={t('lbl.EDITWHO')} maxLength={20} allowClear disabled={true} />
									</li>
								</UiDetailViewGroup>
							</UiDetailViewArea>
						</AGrid>
...
export default MsSkuDcSetDetailButtonPaging;
