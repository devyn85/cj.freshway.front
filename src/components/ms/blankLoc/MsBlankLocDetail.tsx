...
// API
import { apiPostSaveMasterList } from '@/api/ms/apiMsBlankLoc';

const MsBlankLocDetail = forwardRef((props: any, gridRef: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	const gridCol = [
		...
		},
		{
			...
			editRenderer: {
				...
				listFunction: (rowIndex: number, colIndex: number, item: any) => {
					// 물류센터 코드에 따라 피킹존 리스트를 가져온다.
					return getMsZoneList(item.dcCode);
				},
				// list: getMsZoneList(gDccode),
				keyField: 'baseCode', // key 에 해당되는 필드명
				valueField: 'baseDescr',
			},
			filter: {
				showIcon: true,
			},
		...
		{
			dataField: 'locType',
			headerText: t('lbl.LOCTYPE'),
			dataType: 'code',
			editRenderer: {
				type: 'DropDownListRenderer',
				list: getCommonCodeList('LOCTYPE_BLANK', ''),
				keyField: 'comCd', // key 에 해당되는 필드명
				valueField: 'cdNm',
			},
			labelFunction: (rowIndex: any, columnIndex: any, value: any) => {
				return getCommonCodebyCd('LOCTYPE_BLANK', value)?.cdNm;
			},
		},
		{
			dataField: 'rackLine',
			headerText: t('lbl.RACKLINE'),
			dataType: 'numeric',
			maxlength: 10,
			editRenderer: {
				type: 'InputEditRenderer',
		...
		btnArr: [
			{
				...
				initValues: {
					storerkey: 'FW00',
					dcCode: gDccode, // 첫 번째 행의 물류센터 코드로 초기화
					delYn: 'N',
...
export default MsBlankLocDetail;
