/*
 ############################################################################
 # FiledataField	: MsKitUploadExcelPopup.tsx
 # Description		:  Kit상품 엑셀 업로드 팝업
 # Author			: jangjaehyun
 # Since			: 25.07.03
 ############################################################################
*/
...
// API Call Function
import { apiGetValidateExcelList, apiPostSaveExcelList } from '@/api/ms/apiMsKit';

interface PropsType {
	close?: any;
}

const MsKitUploadExcelPopup = (props: PropsType) => {
	/**
	 * =====================================================================
	 *	01. 변수 선언부
	 * =====================================================================
	...
	const validateExcelList = async () => {
		...
		apiGetValidateExcelList(params).then(res => {
			...
			rowsToUpdate.forEach((row: any, index: any) => {
				// const rowIndex = gridRef.current.getRowIndexesByValue('kitSku', [row.kitSku]);
				// if (rowIndex !== undefined) {
				// 	let processMsg;
				// 	let processYn;
				// 	if (row.processYn === 'N') {
				// 		processMsg = '이미 등록된 상품이 있습니다.';
				// 		processYn = 'N';
	...
	const saveExcelList = () => {
		...
		const params = gridRef.current.getCustomCheckedRowItems().map((item: any) => {
			return {
				...
				delYn: 'N',
				// fromDate: item.fromDate ? item.fromDate.replace(/-/g, '') : '',
				// toDate: item.toDate ? item.toDate.replace(/-/g, '') : '',
		...
		showConfirm(null, messageWithRowStatusCount, () => {
			apiPostSaveExcelList(params).then((res: any) => {
				if (res.statusCode > -1) {
					showMessage({
						content: t('msg.MSG_COM_SUC_003'),
						modalType: 'info',
	...
	return (
		<>
			{/* 상단 타이틀 및 페이지버튼 */}
			<PopupMenuTitle name="KIT상품정보 엑셀 업로드" showButtons={false} />
...
export default MsKitUploadExcelPopup;
