import { apiGetRoleList, apiGetRoleMappingUser, apiPostSaveRoleAndUser } from '@/api/common/apiSysmgt';
import AGrid from '@/assets/styled/AGrid/AGrid';
import PageGridBtn from '@/components/common/PageGridBtn';
import { showAlert, showConfirm } from '@/util/MessageUtil';

import AUIGrid from '@/lib/AUIGrid/AUIGridReactCanal';

interface PropsType {
	form?: any;
}

const AuthUserGrid = forwardRef(({ form }: PropsType, ref: any) => {
	const { t } = useTranslation();

	const gridRef1 = useRef(null);
	const gridRef2 = useRef(null);

	useEffect(() => {
		initGrid();
	}, []);

	const searchAuthUser = () => {
		gridRef1.current.clearGridData();

		apiGetRoleList(form.getFieldsValue()).then(res => {
			gridRef1.current.setGridData(res.data);
			gridRef1.current.setSelectionByIndex(0, 0);
		});
	};
	const searchAuthUserDtl = () => {
		gridRef2.current.clearGridData();
		const page = { startRow: 0, listCount: 500, skipCount: false };
		let params = {};
		const selectedRow = gridRef1.current.getSelectedRows();
		if (selectedRow.length > 0 && !gridRef1.current.isAddedById(selectedRow[0]._$uid)) {
			params = {
				...form.getFieldsValue(),
				authority: selectedRow[0].authority,
				...page,
			};

			apiGetRoleMappingUser(params).then(res => {
				gridRef2.current.setGridData(res.data?.list);
			});
		}
	};

	const saveAuthUser = () => {
		const roleUsers = gridRef2.current.getChangedData();
		if (!roleUsers || roleUsers.length < 1) {
			showAlert(null, t('com.msg.noChange')); // 변경사항이 없습니다
			return;
		}

		//저장하시겠습니까
		showConfirm(null, t('com.msg.confirmSave'), () => {
			const params = {
				roleUsers: roleUsers,
			};
			apiPostSaveRoleAndUser(params).then(() => {
				searchAuthUser();
			});
		});
	};

	const gridCol1 = [
		{
			dataField: 'authority',
			headerText: t('sysmgt.roles.group.authority'),
		},
		{
			dataField: 'roleNm',
			headerText: t('sysmgt.roles.group.roleNm'),
		},
		{
			dataField: 'description',
			headerText: t('sysmgt.roles.group.description'),
		},
	];

	const gridProps1 = {
		editable: false,
	};

	const gridBtn1 = {};

	const gridCol2 = [
		{
			headerText: t('sysmgt.rolesmappingusers.search.include'),
			dataField: 'include',
			commRenderer: {
				type: 'checkBox',
			},
			width: '80',
		},
		{
			// 사용자명
			dataField: 'userNm',
			headerText: t('com.col.userNm'),
			width: '150',
			editable: false,
		},
		{
			// 사용자ID
			dataField: 'userId',
			headerText: t('com.col.userId'),
			width: '150',
			editable: false,
		},
		{
			// 등록자ID
			dataField: 'regId',
			headerText: t('com.col.regId'),
			width: '150',
			editable: false,
		},
		{
			// 등록일자
			dataField: 'regDt',
			headerText: t('com.col.regDt'),
			width: '150',
			editable: false,
		},
	];

	const gridProps2 = {
		editable: true,
		showStateColumn: true,
	};

	const gridBtn2 = {};

	const initGrid = () => {
		const gridRefCur1 = gridRef1.current;
		gridRefCur1.bind('selectionChange', function () {
			// 상세코드 조회
			searchAuthUserDtl();
		});
	};
	useImperativeHandle(ref, () => ({ searchAuthUser, saveAuthUser }));

	return (
		<>
			<AGrid>
				<PageGridBtn gridBtn={gridBtn1} gridTitle={t('sysmgt.rolesmappingusers.common.auth')} />
				<AUIGrid ref={gridRef1} columnLayout={gridCol1} gridProps={gridProps1} />
			</AGrid>
			<AGrid>
				<PageGridBtn gridBtn={gridBtn2} gridTitle={t('sysmgt.rolesmappingusers.common.user')} />
				<AUIGrid ref={gridRef2} columnLayout={gridCol2} gridProps={gridProps2} />
			</AGrid>
		</>
	);
});
export default AuthUserGrid;
