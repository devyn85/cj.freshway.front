...
// API Call Function
import { apiGetCmmCdSearchCmmCd } from '@/api/common/apiSysmgt';

const DetailCommonCode = forwardRef((props: any, ref: any) => {
	/**
	 * =====================================================================
	 *  01. 변수 선언부
	 * =====================================================================
	...
	*/
	...
	const gridBtn = {
		...
		minusFunction: function () {
			...
			if (gridRefCur1.isAddedById(selectedRow[0][rowIdField])) {
				ref.gridRef1.current.removeRow('selectedIndex');
			} else {
	...
	const gridBtn2 = {
		...
		plusFunction: function () {
			...
			ref.gridRef2.current.addRow(item);
		},
		minusFunction: function () {
			...
			if (gridRefCur2.isAddedById(selectedRow[0][rowIdField])) {
				ref.gridRef2.current.removeRow('selectedIndex');
			} else {
				showAlert(null, t('com.msg.newDelSelect')); //신규행만 삭제가 가능합니다
				return;
			}
		...
		copyFunction: function () {
			...
			if (selectedRow && selectedRow.length > 0) {
				...
				item.comCd = '';
				ref.gridRef2.current.addRow(item, 'selectionDown');
			}
		},
	};
	 ...
	 *  02. 함수
	 * =====================================================================
	 * @param {string} comGrpCd 공통 그룹 코드
	 * @returns {void}
	 */
...
export default DetailCommonCode;
